...
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------ | --------------- |
| `prepare_online.sh`    | Tworzy strukturę `~/SecureBoot_Project`, pobiera obrazy ISO (Ubuntu, Pop!_OS, DBAN), pakiety `.deb` i narzędzia (Etcher) do pracy nad Secure Boot. | 1_1_prepare_online.sh    | 3.1             |
...
| `back_online.sh`       | Przywraca połączenia Wi-Fi/Ethernet, odtwarza konfigurację sieci i testuje łączność.                                                               | 1_3_verify_offline.sh    | 3.3             |
| `verify_offline.sh`    | W trybie offline sprawdza integralność pobranych plików (ISO, narzędzi, pakietów) i potwierdza brak sieci.                                         | 1_4_back_online.sh       | 3.3             |
| `create_pendrive_A.sh` | Formatuje Pendrive A (Ubuntu Live + tools), nagrywa ISO i kopiuje pakiety (`efitools`, `sbsigntool`, `openssl`, `balenaEtcher.AppImage`).          | 1_5_create_pendrive_A.sh | 3.4             |
